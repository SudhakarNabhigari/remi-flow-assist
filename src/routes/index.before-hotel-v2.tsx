...
export const Route = createFileRoute("/index/before-hotel-v2")({
  head: () => ({
    meta: [
      { title: "RimeFlow â€” Talk to Remi, your real-time voice assistant" },
      {
        name: "description",
        content:
          "RimeFlow is a real-time multilingual voice assistant with instant interruption handling, powered by Rime speech.",
      },
      { property: "og:title", content: "RimeFlow â€” Talk to Remi" },
      {
        property: "og:description",
        content: "Speak naturally in English, Telugu or Hindi. Interrupt any time â€” Remi adapts instantly.",
      },
  ...
  return (
    <>
      ...
      <header className={greeted ? "animate-fade-in text-center" : "text-center opacity-0"}>
        <p className="text-xs font-semibold uppercase tracking-[0.32em] text-primary/70">RimeFlow Â· Live</p>
        <h1 className="mt-3 text-3xl font-black tracking-tight md:text-5xl">
          Hello, <span className="text-gradient-blue">{displayName}</span>
        </h1>
        <p className="mt-3 max-w-md text-sm text-muted-foreground md:text-base">
          Say â€œhey {settings.nickname}â€ to start. Speak English, Telugu or Hindi â€” I answer in whichever
      ...
      <div className="mt-2 min-h-[3.5rem] w-full max-w-xl text-center">
        {engine.partial ? (
          <p className="animate-fade-in text-sm text-muted-foreground">â€œ{engine.partial}â€</p>
        ) : engine.lastReply ? (
          <p className="animate-fade-in text-base font-medium">{engine.lastReply}</p>
        ) : (
          <p className="text-sm text-muted-foreground">Try: â€œhey {settings.nickname}, {lang.sampleUtterance}â€</p>
        )}
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-2 text-xs text-muted-foreground">
        ...
        <Badge>
          ...
          Auto language Â· {getLanguage(engine.spokenLanguage).label}
        </Badge>
        <Badge>
        ...
        {engine.providerInfo.provider && (
          <Badge>
            ...
            {engine.providerInfo.speaker ? ` Â· ${engine.providerInfo.speaker}` : ""}
          </Badge>
        )}
      </div>
