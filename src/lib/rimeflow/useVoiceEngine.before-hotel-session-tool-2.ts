...
const HOTEL_SELECTION_COMMAND =
  /\b(?:select|choose|pick|recommend)\b.*(?:hotel|hotels|one|any|best|this|that)|\b(?:select|choose|pick)\b.*(?:from|among)\b/i;

const HOTEL_OPEN_COMMAND =
  /\b(?:open|visit|show me|go to|take me to)\b.*(?:hotel|it|this one|that one|selected one|website|booking|page)\b|\b(?:open|visit)\s+(?:it|this|that)\b/i;
