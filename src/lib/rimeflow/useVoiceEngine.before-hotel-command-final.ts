...
const HOTEL_SELECTION_COMMAND =
  /\b(?:select|choose|pick)\b.*\b(?:hotel|hotels|one|any|best|top|suggestion|option)\b|\b(?:select|choose|pick)\b.*\b(?:from|among)\b|\b(?:select|choose|pick)\b\s+(?:the\s+)?(?:best|top)?\s*(?:one|hotel|option)\b/i;

const HOTEL_OPEN_COMMAND =
  /\b(?:open|visit|show me|go to|take me to)\b.*\b(?:hotel|it|this one|that one|selected one|website|booking|page|site)\b|\bopen\b\s+(?:it|this|that)\b/i;

const HOTEL_CLOSE_COMMAND =
  /\b(?:close|exit|hide|stop)\b.*\bhotel(?:\s+mode)?\b|\bhotel(?:\s+mode)?\b.*\b(?:close|exit|hide|stop)\b/i;
