  ...
  .handler(async ({ data }) => {
    ...
    if (catalog.reachable) {
      ...
      if (speaker && !catalog.speakers.includes(speaker)) {
        ...
        availabilityNote =
          `UNAVAILABLE WITH CURRENT RIME CONFIGURATION ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â "${speaker}" is not in the connected Rime catalogue.`;
      ...
      availabilityNote =
        "UNAVAILABLE WITH CURRENT RIME CONFIGURATION ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â custom/cloned voices depend on the connected Rime account.";
    }
...
export const agentReply = createServerFn({
  ...
  .handler(async ({ data }) => {
    ...
    const system = [
      `You are ${data.nickname}, the voice of RimeFlow ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â a realtime voice assistant.`,
