...
const isLikelyAssistantEcho = (candidate: string, assistantText: string) => {
  const candidateNormalized = normalizeSpeechForEcho(candidate);
  const assistantNormalized = normalizeSpeechForEcho(assistantText);

  if (!candidateNormalized || !assistantNormalized) return false;
  if (candidateNormalized.length < 8) return false;

  const candidateWords = candidateNormalized.split(" ");
  if (candidateWords.length < 2) return false;

  if (
    candidateNormalized === assistantNormalized ||
    assistantNormalized.startsWith(candidateNormalized)
  ) {
    return true;
  }

  const assistantWords = new Set(assistantNormalized.split(" "));
