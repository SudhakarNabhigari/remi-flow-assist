...
export function createBrowserStt(
  ...
  const build = (myGeneration: number): RecognitionLike => {
    ...
    r.onend = () => {
      ...
      /*
       * Chrome may end an individual SpeechRecognition session even while
       * the voice engine is still running. Keep the UI in MIC ON state while
       * we recreate the recognition session.
       */
      if (running) {
...
/**
 * Wake-phrase matcher Ã¢â‚¬â€ works for any nickname the user sets.
 *
 * Examples:
 *   "hey nayak"
 *   "hi nayak"
